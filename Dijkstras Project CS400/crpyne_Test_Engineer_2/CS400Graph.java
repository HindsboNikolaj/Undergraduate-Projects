...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
            // Done: Implement this constructor in Step 5.
        	this(source.start);
    ...
    protected Path dijkstrasShortestPath(T start, T end) {  
    	if(start.equals(end)) return new Path(vertices.get(start));
    	if(!vertices.containsKey(start)||!vertices.containsKey(end)) throw new NoSuchElementException("Start or end is not in graph");
    	
        PriorityQueue<Path> queue = new PriorityQueue();
        Path current = new Path(vertices.get(start));
        ...
        outer : while(!queue.isEmpty()) {
        	...
        	if(visited.put(current.end.data, current.end) == null) {
        		
        		//No negative edges, so when current distance is greater than shortest distance leave the loop.
        		if(!(shortestPath == null)&&current.compareTo(shortestPath) > 0) break outer;
        		
		    	for(Edge currentEdge: current.end.edgesLeaving) {
		    		//makes sure visited nodes are ignored
		    		if(!visited.contains(currentEdge.target)) {
		    			Path newPath = new Path(current);
		    			newPath.extend(currentEdge);
		    			queue.add(newPath);
		    			
		    			//updates shortest path when a shorter path is found
		    			if(newPath.end.data.equals(end) && (shortestPath == null || newPath.compareTo(shortestPath) < 0)) {
        ...
        if(shortestPath == null) throw new NoSuchElementException("No path exists");
        return shortestPath;
