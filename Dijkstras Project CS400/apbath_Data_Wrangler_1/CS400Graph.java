...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path source) {
			this.start = source.start; // first vertex within path
			this.distance = source.distance; // sumed weight of all edges in path

			this.dataSequence = new LinkedList<>(source.dataSequence); // ordered sequence of data from vertices in path

			this.end = source.end; // last vertex within path
...
//        	    }
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		...
		// Make sure that you keep track of which vertices you have already found
		LinkedList<Vertex> visited = new LinkedList<>();

		// hold the discovered paths (including total cost distance, and previous vertex
		// data) that have not
		// yet been determined to be the shortest possible paths to their given end
		// vertex.
		PriorityQueue<Path> frontier = new PriorityQueue<>();

		if (!vertices.containsKey(start) || !vertices.containsKey(end)) {
			throw new NoSuchElementException("start or end doesn't exist");
		}

		Vertex startVert = vertices.get(start);
		Vertex endVert = vertices.get(end);

		Path first = new Path(startVert);
		frontier.add(first);

		while (!frontier.isEmpty()) {

			if (frontier.peek().end.equals(endVert)) {
				Path returned = frontier.poll();
				return returned;
			}

			// .poll to find lowest cost
			Path holder = frontier.poll();
			// set visited to true
			visited.add(holder.end);

			// for each unvisited successor
			// public LinkedList<Edge> edgesLeaving;
			for (i = 0; i < holder.end.edgesLeaving.size(); i++) {
				// already visited?
				Vertex findIfVisited = holder.end.edgesLeaving.get(i).target;
				Edge edgeToAdd = holder.end.edgesLeaving.get(i);
				Path extended = new Path(holder);
				extended.extend(edgeToAdd);

				if (!visited.contains(findIfVisited)) {
					Path modify = new Path(holder);
		...
		throw new NoSuchElementException("Could not find connected path");
