...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
            ...
            //copy in vertices from old into new
            this.dataSequence = new LinkedList<>();
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
        ...
        Hashtable<Vertex, Integer> weights = new Hashtable<Vertex, Integer>();

        //initialize all weights to infinity
        for (Map.Entry<T, Vertex> v : vertices.entrySet()) {
            weights.put(v.getValue(), Integer.MAX_VALUE);
        }

        //insert into priority queue
        pq.add(new Path(startVert));
        weights.put(startVert, 0);

        //while pq not empty: do the thing
        while (!pq.isEmpty()) {
            Path c = pq.remove();
            if (c.end == vertices.get(end)) {
                return c;
            } else {

                visited.add(c.end);
                //for every neighbor of c, see if you can update
                for (Edge e : c.end.edgesLeaving) {
                    if (!visited.contains(e.target)) {
                        //check if we can update total
                        if (weights.get(e.target) > (c.distance + e.weight)) {
                            //update vertex and append path
                            weights.put(e.target, c.distance + e.weight);
                            Path pain = new Path(c);
                            pain.extend(e);
                            pq.add(pain);
                        }
                    }
