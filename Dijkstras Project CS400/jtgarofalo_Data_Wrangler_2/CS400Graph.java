...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
        	...
        	List<T> newList = source.dataSequence;
        	this.start = source.start;
        	this.distance = source.distance;
        	this.dataSequence = newList;
        	this.end = source.end;
        }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {   
    	...
    	//Initialize variables, priority queue, vertices, HashTable, etc..
    	Hashtable<Vertex, Path> shortPath = new Hashtable<>();
    	Vertex startVer = vertices.get(start);
    	Vertex endVer = vertices.get(end);
    	int capacity = vertices.size();
    	Path startPath = new Path(startVer);
    	PriorityQueue<Path> queue = new PriorityQueue(capacity);
    	queue.add(startPath);
    	
    	//begin loop that will  find shortest path
    	while(!queue.isEmpty()) {
    		Path tempPath = queue.poll();
    		Vertex tempVer = tempPath.end;
    		if(tempVer == endVer) {
    			return tempPath;
    		}
    		int edgeList = tempVer.edgesLeaving.size();
    		
    		//Record Visited node in Hashtable
    		shortPath.put(tempVer, tempPath);
    		
    		for(int i = 0; i < edgeList; i++) {
    			Vertex ver = tempVer.edgesLeaving.get(i).target;
    			Edge edge = tempVer.edgesLeaving.get(i);
    			//only takes unvisited vertexes into account
    			if(!shortPath.containsKey(ver)) {
    				Path copyPath = new Path(tempPath);
    				copyPath.extend(edge);
    				queue.add(copyPath);
    			}
    		}
    	...
    	//Throw exception in the event that no path was found
    	throw new java.util.NoSuchElementException();
