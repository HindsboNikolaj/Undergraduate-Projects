...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {  
        ...
        if (!containsVertex(end) || !containsVertex(start))
          throw new NoSuchElementException("Data does not exsist in the graph");
        ...
        que.add(new Path(vertices.get(start)));
        ...
        while (!que.isEmpty()) {
          ...
          //if (curr != null && curr.end != null && curr.end.edgesLeaving != null); {
            for (int i = 0; i < curr.end.edgesLeaving.size(); i++) {
              
              if (curr.end.edgesLeaving.get(i).target.data == end) {
                curr.extend(curr.end.edgesLeaving.get(i));
                return curr;
              }
              
              if (!visited.contains(curr.end.edgesLeaving.get(i).target.data) ) {
                newEntry = new Path(curr);
                newEntry.extend(newEntry.end.edgesLeaving.get(i)); //THIS ONE
                visited.put(curr.end.edgesLeaving.get(i).target.data.hashCode(), curr.end.edgesLeaving.get(i).target.data);
                que.add(newEntry);
              }
            }
          //}
        ...
        throw new NoSuchElementException("No path to the end node");
    }
